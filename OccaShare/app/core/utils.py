...
def is_valid_business_name(name: str) -> Optional[str]:
    ...
    if not re.match(r'^[a-zA-Z0-9\s\.\'\-\,\&]+$', name):
        return "Invalid characters in business name"
...
def is_dummy_phone(phone: str) -> Optional[str]:
    ...
    if re.search(r'(.)\1\1', phone):
        return "Mobile number contains too many repetitive digits"
    if re.search(r'(.{2,3})\1\1', phone):
        return "Mobile number contains repetitive patterns"
        ...
        try:
            ...
            while query:
                ...
                if data.get('status') == 'OK' and data.get('results'):
                    ...
                    db.commit()
                    print(f"[Geocode] Successfully geocoded Caterer {caterer_id} to {loc['lat']}, {loc['lng']} using query: {query}")
                    break
                else:
                    parts = query.split(',')
                    if len(parts) > 2:
