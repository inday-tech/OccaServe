...
async def upload_file_to_supabase(
    ...
    """
    if not supabase:
        print("Warning: Supabase client is not initialized. Make sure SUPABASE_URL and SUPABASE_KEY are set.")
        return None
...
async def delete_file_from_supabase(url: str, bucket: str = DEFAULT_BUCKET) -> bool:
    ...
    try:
        ...
        if bucket_public_path in url:
            ...
            response = supabase.storage.from_(bucket).remove([path_in_bucket])
            return True
        return False
    except Exception as e:
