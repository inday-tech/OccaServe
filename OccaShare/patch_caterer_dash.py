...
if "business_hours_open_time" not in content:
    content = content.replace("user: models.User = Depends(caterer_only)", params_to_insert + "    user: models.User = Depends(caterer_only)")
    content = content.replace("profile = user.caterer_profile", "profile = user.caterer_profile" + logic_to_insert)
