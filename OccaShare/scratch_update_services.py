...
{% set categories_list = ['Tables', 'Chairs', 'Tent', 'Stage', 'Lights and Sounds', 'LED Wall', 'Flowers', 'Backdrops', 'Balloons', 'Centerpieces', 'Drapes', 'Waiter', 'Host', 'Coordinator', 'Stylist', 'Photographer', 'Videographer', 'Other'] %}
    ...
    <div class="menu-item-card-premium" data-category="{{ item.category }}" data-name="{{ item.name|lower }}">
        ...
        <div class="item-info-premium">
            <div class="item-header-row">
                ...
                <span class="item-addon-tag" style="background: #e2e8f0; color: #475569;">{{ item.item_type }}</span>
            </div>
            ...
            <div class="item-footer-premium">
                ...
                <div class="margin-stack">
                    {% set profit = ((item.price or 0) - (item.cost_price or 0)) %}
                    <span class="price-label">Est. Profit</span>
                    <span class="margin-value {{ 'negative' if profit < 0 else 'positive' if profit > 0 else 'neutral' }}">
                        ₱{{ "{:,.0f}".format(profit) }}
                    </span>
                </div>
            </div>
    ...
    <div class="occ-modal-box sz-md occ-content-pop">
        ...
        <form id="menuForm" action="/caterer/services/add" method="POST" enctype="multipart/form-data" novalidate>
            ...
            <div class="occ-modal-body compact-body" style="max-height: 70vh; overflow-y: auto;">
                ...
                <div class="form-row-pro">
                    <div class="form-group-pro">
                        <label>Estimated Cost (₱) * <i class="fas fa-info-circle text-muted" title="Internal business cost used for ROI"></i></label>
...
<script>
    ...
    const equipmentCats = ['Tables', 'Chairs', 'Tent', 'Stage', 'Lights and Sounds', 'LED Wall', 'Other'];
    const decorationCats = ['Flowers', 'Backdrops', 'Balloons', 'Centerpieces', 'Drapes', 'Other'];
    const serviceCats = ['Waiter', 'Host', 'Coordinator', 'Stylist', 'Photographer', 'Videographer', 'Other'];

    const equipmentUnits = ['Piece', 'Set'];
    const decorationUnits = ['Piece', 'Set'];
    const serviceUnits = ['Person', 'Event'];
    ...
    function handleTypeChange() {
        ...
        if (type === 'Equipment') { cats = equipmentCats; units = equipmentUnits; }
        else if (type === 'Decoration') { cats = decorationCats; units = decorationUnits; }
        else if (type === 'Service') { cats = serviceCats; units = serviceUnits; }
    ...
    function openAddMenuModal() {
        ...
        removeImage();
        typeSelect.disabled = false; // Allow changing type for new item
        handleTypeChange();
    ...
    async function editMenuItem(el) {
        ...
        try {
            ...
            // Allow any unit previously saved
            if (!Array.from(unitSelect.options).map(o => o.value).includes(item.pricing_unit)) {
                const opt = document.createElement('option');
                opt.value = item.pricing_unit; opt.innerText = item.pricing_unit;
                unitSelect.appendChild(opt);
            }
            form.unit_type.value = item.pricing_unit || 'Piece';
    ...
    document.getElementById('confirmArchiveBtn').onclick = async () => {
        ...
        const res = await window.apiAction(`/caterer/services/${archiveTargetId}/archive?type=${archiveTargetType}`, { method: 'POST' });
        if (res) {
    ...
    form.addEventListener('submit', async function (e) {
        e.preventDefault();
        typeSelect.disabled = false; // Temporarily enable to get value if disabled
        
        if (!this.checkValidity()) {
            this.reportValidity();
            if (document.getElementById('updateItemType').value) typeSelect.disabled = true;
            return;
        }
        ...
        if (catSelect.value === 'Other' && customCat && customCat.value.trim() !== '') {
            const newOpt = new Option(customCat.value.trim(), customCat.value.trim(), true, true);
        ...
        // Use hidden input for type if updating
        ...
        if (window.apiAction) {
            ...
            else if (updateType) typeSelect.disabled = true; // Restore disabled state on fail
        }
    });
